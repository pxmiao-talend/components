...
/**
 * Error codes related to the Component from the dataset designer perspectve
 */
public enum DatasetsErrorCode implements ErrorCode {
    /** used when a dataset schema has a wrong type */
    SCHEMA_TYPE_MISMATCH(HttpServletResponse.SC_BAD_REQUEST, "dataset", "expected", "current"), //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    /** used when a dataset axpect a schema and none is set */
    SCHEMA_MISSING(HttpServletResponse.SC_BAD_REQUEST, "dataset"), //$NON-NLS-1$
     ...
     */
    DatasetsErrorCode(int httpStatus) {
     ...
     * @param httpStatus the http status to use.
     */
    DatasetsErrorCode(int httpStatus, String... contextEntries) {
